...
if (GVAR(activeSpawn) != GVAR(activeBase)) then {
    _checkSpawn = true;
};

if ((!(((getPos GVAR(activeSpawn)) nearEntities 5) isEqualTo [])) && _checkSpawn) exitWith {
    hint localize "STR_KPCF_HINTZONE";
    [{hintSilent "";}, [], 3] call CBA_fnc_waitAndExecute;
};

// Spawn crate
private _crate = createVehicle [_crateType, ((getPos GVAR(activeSpawn)) findEmptyPosition [0, 10, _crateType]), [], 0, "NONE"];
