...
GVAR(activeBase) = _data select 0;

GVAR(activeSpawn) = nearestObject [getPos GVAR(activeBase), GVAR(cratefillerSpawn)];

if (isNull GVAR(activeSpawn)) then {
    hint localize "STR_KPCF_HINTNOSPAWN";
    [{hintSilent "";}, [], 3] call CBA_fnc_waitAndExecute;
    GVAR(activeSpawn) = GVAR(activeBase);
};
...
if (!GVAR(canSpawnAndDelete)) then {
